...
const StokBarangHelper = () => {
  ...
  const getDataBarangNonPaket = async () => {
    await axios
      .get(`${baseUrl}/stok-barang/jenis/non-paket`, {
        headers: {
          Accept: "Application/json",
          Authorization: `Bearer ${localStorage.getItem("sip-token")}`,
        },
  ...
  const postStokBarang = async () => {
    ...
    if (input.harga_reseller != currentStokBarang.harga_reseller) {
      if (
        input.harga_reseller.indexOf(".") != -1 ||
        input.harga_reseller.indexOf(",") != -1
      ) {
    ...
    const stokCabang = Array.from(
      new Set(inputCabang.map((x) => x.id_cabang))
    ).map((id, i) => {
      return {
        id_cabang: id,
        stok_tersedia: inputCabang[i].stok_tersedia,
        stok_dapat_dijual: inputCabang[i].stok_dapat_dijual,
      };
    });
    ...
    await axios
      .post(
        ...
        {
          nama_barang:
            input.paket == 1 ? `Paket ${input.nama_barang}` : input.nama_barang,
  ...
  const deletePaket = async (id) => {
    await axios
      .delete(`${baseUrl}/paket-barang/${id}`, {
        headers: {
          Accept: "Application/json",
          Authorization: `Bearer ${localStorage.getItem("sip-token")}`,
        },
